...
enum WiFiOnboardingPages
{
    CONNECT_QRCODE_PAGE = 0,
    WEBSERVER_QRCODE_PAGE,
    CONTINUE_IN_BROWSER_WIFI_PAGE,
    CONNECTING_TO_WIFI_PAGE,
    WIFI_ONBOARDING_PAGES_COUNT,
    WIFI_LOADING_PAGE
};

class WiFiLoadingPage : public BasePage
{
public:
    WiFiLoadingPage(lv_obj_t *parent) : BasePage(parent)
    {
...
public:
    WebServerQRCodePage(lv_obj_t *parent) : BasePage(parent)
    {
        ...
        sprintf(buf, "OR OPEN #FFFFFF http://192.168.4.1/?wifi#\n IN YOUR BROWSER");
...
class WiFiOnboardingPageManager : public PageManager<WiFiOnboardingPages>
{
public:
    WiFiOnboardingPageManager(lv_obj_t *parent, SemaphoreHandle_t mutex)
        : PageManager<WiFiOnboardingPages>(parent, mutex)
    {
        add(WIFI_LOADING_PAGE, new WiFiLoadingPage(parent));
...
{
public:
    WiFiOnboardingFlow(SemaphoreHandle_t mutex, RenderParentCallback render_parent);
...
private:
    ...
    WiFiOnboardingPageManager *page_mgr = nullptr;
