...
EntityStateUpdate OnboardingFlow::updateStateFromKnob(PB_SmartKnobState state)
{
    if (current_position != state.current_position)
    {
        ...
        xTaskCreate(screen_load_task, "Screen load task", 1024 * 2, screens[current_position], tskIDLE_PRIORITY, NULL);
    }
...
{
    screens[WELCOME] = lv_obj_create(NULL);
...
{
    screens[HASS_1] = lv_obj_create(NULL);
    lv_obj_set_size(screens[HASS_1], LV_HOR_RES, LV_VER_RES);

    lv_obj_t *label = lv_label_create(screens[HASS_1]);
...
{
    screens[WIFI_1] = lv_obj_create(NULL);
    lv_obj_set_size(screens[WIFI_1], LV_HOR_RES, LV_VER_RES);

    lv_obj_t *label = lv_label_create(screens[WIFI_1]);
...
{
    screens[DEMO_1] = lv_obj_create(NULL);
    lv_obj_set_size(screens[DEMO_1], LV_HOR_RES, LV_VER_RES);

    lv_obj_t *label = lv_label_create(screens[DEMO_1]);
...
{
    screens[ABOUT] = lv_obj_create(NULL);
